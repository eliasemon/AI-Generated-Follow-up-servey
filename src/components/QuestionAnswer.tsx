...
export const QuestionAnswer: React.FC = () => {
  ...
  const generateNextQuestions = async () => {
    ...
    try {
      ...
      if (data?.questions) {
        console.log("here");
        const newQuestions = data.questions.reduce(
          (
            acc: {
              id: string;
              question: string;
  ...
  const randomizeQuestionsPattrens = () => {
    const randomIndex = Math.floor(
      (Math.random() * initialsQuestions.length) / 3
    );
    console.log(randomIndex);
    const randomQuestions = initialsQuestions.slice(
      randomIndex * 3,
      randomIndex * 3 + 3
    );
  ...
  return (
    <>
      <div className="mr-6 max-w-2xl w-full p-6 bg-white rounded-lg shadow-lg">
        <div className="mb-8">
          <h2 className="text-xl font-semibold mb-2">
            {questions[currentIndex].type == "followUp" && "Follow-up"} Question{" "}
        ...
        >
          {currentIndex < questions.length - 1
            ? "Next Question"
            : `Generate ${currentIndex > 3 ? "more" : ""}  Follow Up Questions`}
        </button>
      </div>
